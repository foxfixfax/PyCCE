...
class Pulse:
    ...
    Args:
        which (iterable): Indexes of the central spins to be rotated by the pulse. Default is all.
            Separated indexes are supported only in gCCE and only if qubit states are provided separately for all
            center spins.
    ...
    def __init__(self, axis=None, angle=None, delay=None, which=None,
                 bath_names=None, bath_axes=None, bath_angles=None):
        pi = np.pi
        ...
        if angle is not None and np.isclose(angle, np.pi).all():
            self.flip = True
...
class nPulse(BasePulse):
    ...
    def __init__(self, axis=None, angle=None, delay=None, which=None,
        ...
        super(nPulse, self).__init__(**kwargs)
        if axis is not None:
            self.axis = axis
        """str: Axis of rotation of the central spin"""
        if isinstance(angle, str):
